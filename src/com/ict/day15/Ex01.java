...
/*
 	제네릭과 컬렉션(자료구조)
 	제네릭 : 컬렉션이 어떤 객체들로 이루어졌는지를 표시하는 개체 타입
 	컬렉션 : 객체들을 모아서 저장 관리하는 인터페이스들을 말한다.
 	  ...
 	  클래스 => 배열은 같은 종류의 자료형을 저장하는 것이 불편해서 클래스에 저장하면 
  	  ...
  	  API : <T> => 객체 타입(Type), <E> => 요소(Element), 컬렉션 안에 존재하는 객체 하나 하나를 뜻한다.
  	  		Map 형식 : <K,Y> => Key(키값), Value(데이터값)
  	  		1 : 1 대응(키 하나에 값 하나), ** Key호출하면 Value가 나온다.
  	  최상위 컬렉션 Collection<E>,  Map<K,V>
  	  Collection<E> 를 상속 받아서 실제로 인터페이스들 : Set<E>, List<E>, Queue<E>
	  ...
	  size() : int => 컬렉션안에 존재하는 Element의 수
	  ...
	  *								 Iterator 객체를 반환
	  *								 컬렉션 안에 존재하는 객체들을 하나씩 꺼내서 작업할때 사용 
