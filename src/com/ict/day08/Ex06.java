...
public class Ex06 {
	/*
    	...
    	1) 헤더  :   [접근제한자] [클래스종류] class 클래스이름{} // 대괄호는 필요 없으면 생략가능
       	가) 접근제한자 : 클래스, 멤버필드, 멤버메서드, 생성자 모두 맨 앞에 사용 가능(생략 가능)
   				 외부에서 접근할 수 있는 권한의 단계를 말함
    	  - public : 누구나 다 접근가능
    	  - private : 외부에서는 절대 접근 불가, 내부멤버(클래스안에 있는 멤버필드와 메서드)끼리는 접근가능
    	...
    	5. 멤버메서드 : 동작, 기능
     	...
     	- 해당 메서드를 호출하면 내용이 실행 된 후 ** 내용이 끝나면 호출한 곳으로 되돌아 간다.
    	  ...
    	  // 메인 메서드 : 해당 메서드를 실행한 후에 호출한 JVM 에게 되돌아 가는데 아무것도 가져가지 않는다.
       ...
       8. static과 instance : 멤버필드, 멤버메서드에 공통으로 해당 되는 사항
       ...
       - 힙(heap) : 프로그램이 시작할때는 있거나 중간에 만들어졌다가
       			   어느 순간에는 없어진다. (동적할당)
       			   자기할일을 다하면 사라진다.
