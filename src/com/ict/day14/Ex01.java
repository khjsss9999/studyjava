	 ...
	 // 내부클래스 : 클래스 안에 다른 클래스가 정의 되어 있는 클래스 
	    종류 : Member(멤버), Local(지역), static, Anonymous(익명) 
	 	1. Member
	 	 ...
	 	 특징) 내부클래스는 외부클래스의 멤버필드와 멤버메서드를 마음대로 사용 가능
	 		 단, 상속은 private 접근안됨, 내부클래스는 private 접근가능
	    ...
	    특징) 외부클래스의 메서드 안에 내부클래스가 존재
	 	 ...
	 	 특징) Member 내부클래스와 같은 구조를 가지고 있음
	 	 	 내부클래스의 멤버변수 중 하나라도 static 있으면 
	 	 	 내부클래스를 static으로 만들어야 된다.
	 	 	 그냥 static 접근 방법을 사용하면 된다.(클래스이름.변수)
	 	 	 (외부클래스를 생성하지 않아도 된다.)
	 	 	 
	 	   4. Anonymous(익명) 내부클래스 : 정의된 클래스의 이름이 없는 클래스
	 	   							        다시 호출 할 수 없다.
