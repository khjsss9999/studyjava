...
/*
  List 인터페이스 : 배열과 흡사한 구조,
   ...
   add, push, addElement => 추가 (뒤에 추가로 붙는 것)
   add(index, E) => 삽입 (index는 위치값.해당 위치 요소를 뒤로 밀고 그 위치로 들어가는 것)
   pop  : 맨 위에 존재하는 객체 반환 후 삭제한다.
   peek : 맨 위에 존재하는 객체 반환 후 삭제를 하지 않는다.
   contains(Element e) : boolean => 해당 Element 가 포함 되어있으면 true
   search : 검색(오른쪽에서 시작하는데 처음을 1이라고 설정하고 1부터)
   **indexOf : 검색 (왼쪽부터 시작하는데 처음을 0이라고 설정하고 0부터) => 배열 형식 처럼
   elementAt(index) : 해당 요소 반환
   **get(index) : 해당 요소 반환 
   firstElement() : 맨 처음 요소 반환
   lastElement() : 맨 마지막 요소 반환
   setElement(Element, index) : 치환 (바꿔치기?)
...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 마지막 객체 보기 : pop(반환 후 삭제), peek, lastElement
		String res = s1.peek();
		...
		s1.addElement("마이콜");
		System.out.println(s1);
		System.out.println(s1.size()); // s1에 담긴 것의 개수
		...
		while (!s1.isEmpty()) {
			String k = s1.pop();
			System.out.println(k + ", 크기는 " + s1.size()+"이다."); // pop으로 끄집어내고 size를 사용해서 개수를 확인하니 끄집어낸 다음 개수가 출력됨
		}
	}
}
